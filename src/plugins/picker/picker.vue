            ...
            <div class="wheel-wrapper" ref="wheelWrapper">
              <div class="wheel" v-for="(data, i) in pickerData" :key="i">
                <ul class="wheel-scroll">
                  <li v-for="(item, index) in data" class="wheel-item" :key="index">{{item.label}}</li>
                </ul>
              </div>
            </div>
          </div>
  ...
  methods: {
    confirm() {
      ...
      for (var i = 0; i < this.pickerData.length; i++) {
        ...
        result.push({
          label: this.pickerData[i][index].label,
          value
        });
      }
      this.$emit("select", result);
      if (changed) this.$emit("valuechange", result);
    },
    cancel() {
      this.hide();
      this.$emit("cancel");
